#!/usr/bin/env python3
"""
GPU benchmarking runner for cuVS CAGRA.
Consumes the first line of gpu-runs-cuvs_cagra.txt (CSV: dim,points,k) and appends results
to gpu_cuvs_cagra_performance.csv (one line per run), then removes the processed run.
"""
...
SEED = 42
RUNS_FILE = "gpu-runs-cuvs_cagra.txt"
OUTPUT_CSV = "gpu_cuvs_cagra_performance.csv"
...
def read_next_run(path: str) -> Tuple[Tuple[int, int, int], int]:
    ...
    while idx < len(lines) and not lines[idx].strip():
        idx += 1
    if idx >= len(lines):
        raise RuntimeError("No runs left in gpu-runs-cuvs_cagra.txt")
...
def main() -> None:
    ...
    while True:
        ...
        print(f"Running cuVS CAGRA benchmark: dim={dim}, points={points}, k={k}")
        data_np = load_data(points, dim)
        time_ms, status = time_cuvs_cagra_gpu(data_np, k)
